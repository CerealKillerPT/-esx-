...
AddEventHandler('esx_drugs:hasEnteredMarker', function(zone)
        ...
        --xanax
        if zone == 'XanaxFarm' then
            -- if myJob ~= "police" then
            --     CurrentAction     = 'xanax_harvest'
        ...
        if zone == 'XanaxTreatment' then
            -- if myJob ~= "police" then
            --     if xanaxQTE >= 5 then
            --         CurrentAction     = 'xanax_treatment'
        ...
        if zone == 'XanaxResell' then
            -- if myJob ~= "police" then
            --     if xanax_poochQTE >= 1 then
            --         CurrentAction     = 'xanax_resell'
        ...
        --Heroin
        if zone == 'HeroinFarm' then
            -- if myJob ~= "police" then
            --     CurrentAction     = 'heroin_harvest'
            --     CurrentActionMsg  = _U('press_collect_heroin')
            --     CurrentActionData = {}
            -- end
            CurrentAction     = 'heroin_harvest'
            CurrentActionMsg  = _U('press_collect_heroin')
        ...
        if zone == 'HeroinTreatment' then
            -- if myJob ~= "police" then
            --     if heroinQTE >= 5 then
            --         CurrentAction     = 'heroin_treatment'
            --         CurrentActionMsg  = _U('press_process_heroin')
            --         CurrentActionData = {}
            --     end
            -- end
            if heroinQTE >= 5 then
        ...
        if zone == 'HeroinResell' then
            -- if myJob ~= "police" then
            --     if heroin_poochQTE >= 1 then
            --         CurrentAction     = 'heroin_resell'
            --         CurrentActionMsg  = _U('press_sell_heroin')
            --         CurrentActionData = {}
            --     end
            -- end
            if heroin_poochQTE >= 1 then
...
AddEventHandler('esx_drugs:hasExitedMarker', function(zone)
        ...
        TriggerServerEvent('esx_drugs:stopHarvestXanax')
        TriggerServerEvent('esx_drugs:stopTransformXanax')
        TriggerServerEvent('esx_drugs:stopSellXanax')
        ...
        TriggerServerEvent('esx_drugs:stopHarvestHeroin')
        TriggerServerEvent('esx_drugs:stopTransformHeroin')
        TriggerServerEvent('esx_drugs:stopSellHeroin')
end)
...
AddEventHandler('esx_drugs:ReturnInventory', function(xanaxNbr, xanaxpNbr, methNbr, methpNbr, weedNbr, weedpNbr, heroinNbr, heroinpNbr, jobName, currentZone)
    xanaxQTE       = xanaxNbr
    ...
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) then
               if CurrentAction == 'xanax_harvest' then
                    TriggerServerEvent('esx_drugs:startHarvestXanax')
                end
              if CurrentAction == 'xanax_treatment' then
                    TriggerServerEvent('esx_drugs:startTransformXanax')
                end
                if CurrentAction == 'xanax_resell' then
                    TriggerServerEvent('esx_drugs:startSellXanax')
                end
                ...
                if CurrentAction == 'heroin_harvest' then
                    TriggerServerEvent('esx_drugs:startHarvestHeroin')
                end
                if CurrentAction == 'heroin_treatment' then
                    TriggerServerEvent('esx_drugs:startTransformHeroin')
                end
                if CurrentAction == 'heroin_resell' then
                    TriggerServerEvent('esx_drugs:startSellHeroin')
                end
                CurrentAction = nil
            end
        end
